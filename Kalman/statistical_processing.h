...
float float_mean_filter(std::vector<float> const & vec_in);

float float_sigma_filter(std::vector<float> const & vec_in, float n_sigma=2.0);
...
template <typename T>
T accurate_sigma_filter(std::vector<T> const & vec_in, double n_sigma=2.0, bool verbose=false){
